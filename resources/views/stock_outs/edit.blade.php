<x-app-layout>
    ...
    <div class="min-h-screen bg-[#002147] py-10">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <!-- Title -->
            <h2 class="text-2xl font-semibold text-white mb-6">Edit Stock Out</h2>
            ...
            <!-- Form -->
            <form id="stockOutForm" action="{{ route('stock-outs.update', $stockOut) }}" method="POST" class="bg-white p-8 rounded shadow border-2 border-[#002147] space-y-5">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <select name="item_id" class="w-full border rounded px-3 py-2" required>
                        @foreach ($items as $item)
                            <option value="{{ $item->id }}" {{ $item->id == $stockOut->item_id ? 'selected' : '' }}>
                                {{ $item->name }}
                            </option>
                        @endforeach
                    </select>
