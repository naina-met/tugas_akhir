<x-app-layout>
    ...
    <div class="min-h-screen bg-[#002147] py-10">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <!-- Header Button -->
            <div class="mb-6">
                <a href="{{ route('stock-outs.create') }}" class="bg-[#ff5c10] text-white px-5 py-2 rounded shadow hover:bg-[#a6240d] transition">
                    + Add Stock Out
                </a>
                <a href="{{ route('export.stockouts') }}" class="bg-green-600 text-white px-5 py-2 rounded shadow hover:bg-green-700 ml-2 transition" onclick="return confirmExport();">
                    Export Excel
                </a>
            </div>
                ...
                <table class="w-full text-left border-collapse border-b-2 border-[#002147]">
                    <thead class="bg-[#ff5c10] text-white border-b-2 border-[#002147]">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-sm font-semibold border-r-2 border-[#002147]">Destination</th>
                    ...
                    <tbody class="text-[#002147]">
                        @forelse ($stockOuts as $index => $stockOut)
                            <tr class="border-t-2 border-[#002147] hover:bg-[#f0f4f8] transition">
                                ...
                                <td class="px-6 py-4 border-r-2 border-[#002147]">{{ $stockOut->date }}</td>
                                <td class="px-6 py-4 border-r-2 border-[#002147]">{{ $stockOut->item->name }}</td>
                                <td class="px-6 py-4 border-r-2 border-[#002147]">{{ $stockOut->quantity }}</td>
                                <td class="px-6 py-4 border-r-2 border-[#002147]">{{ $stockOut->outgoing_destination }}</td>
                                <td class="px-6 py-4 border-r-2 border-[#002147]">{{ $stockOut->user->username }}</td>
                                <td class="px-6 py-4 flex gap-2">
                                    <a href="{{ route('stock-outs.edit', $stockOut) }}" class="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1 rounded text-sm">Edit</a>
                                    <form action="{{ route('stock-outs.destroy', $stockOut) }}" method="POST" onsubmit="return confirm('Are you sure?');">
                                        @csrf
                                        @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="text-center px-6 py-8 text-gray-500">No stock-out records found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
